...
/*   Updated: 2020/01/08 14:08:07 by nmbabazi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_write(int fd, char **line, char *temp, int *ret)
{
	...
	while (j == 0 && (*ret = read(fd, buf, BUFFER_SIZE)))
	{
		...
		while (buf[i] && j == 0)
		{
			if (buf[i] == '\n')
			{
				temp = ft_substr(buf, i, BUFFER_SIZE - i);
				buf[i] = '\0';
				j++;
			}
			i++;
...
char	*ft_long(char **line, char *temp, int *ret)
{
	if (*temp == '\n')
		temp++;
	if (*temp == '\n')
		*line = ft_substr(temp, 0, 0);
	else if (*temp != '\n')
		*line = ft_substr(temp, 0, ft_lenline(temp));
	while (temp && *temp != '\n')
		temp++;
	*ret = 1;
	return (temp);
